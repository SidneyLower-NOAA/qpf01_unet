...
class COND_UNET(nn.Module):
    ...
    def __init__(
        ...
        pos_emb_dim=16,
        grid_size=(1597, 2345),
        kernel_depth=[32, 64, 128, 256, 512],
        kernel_size=3,
        dropout_factor=0.0,
    ):
        ...
        divisor = 2 ** (
            len(kernel_depth) - 1
        )  # want grid divisible by 2 for number of pooling layers
        ...
        for l_idx in range(len(kernel_depth) - 2):
            self.downs.append(
                DoubleConv(kernel_depth[l_idx], kernel_depth[l_idx + 1], kernel_size)
            )
        ...
        self.bottleneck = FiLM_DoubleConv(
            kernel_depth[-2],
            kernel_depth[-1],
            time_embedding_dim,
            kernel_size,
        ...
        for l_idx in range(len(kernel_depth) - 1, 0, -1):
            ...
            self.increase_res.append(
                nn.Sequential(
                    nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True),
                    nn.Conv2d(
                        kernel_depth[l_idx], kernel_depth[l_idx - 1], kernel_size=1
                    ),
                )
            )
            self.deconvolve.append(
                FiLM_DoubleConv(
                    kernel_depth[l_idx],
                    kernel_depth[l_idx - 1],
                    time_embedding_dim,
                    kernel_size,
                    dropout_factor,
                )
