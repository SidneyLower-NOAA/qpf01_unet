...
tdlpack_fileout = os.environ.get("FORT52")
...
if global_rank == 0:
    ...
    print(f"              NetCDF output file: {netcdf_fileout}")
...
# grid info
config_ = read_yaml_config([grid_config, prdgn_config], "co")
...
with torch.no_grad():
    for inputs, nbm_qpf06, time_vector, nbm_qpf06_lead_time in tqdm(nbm_loader):
        ...
        outputs_qpf01 = (
            nbm_qpf06[:, :, ny, nx] * outputs_qpf01_prop[:, :, ny, nx].detach()
        )
...
output_gather_list = [
    torch.zeros_like(outputs_collated) for _ in range(dist.get_world_size())
]
lead_time_gather_list = [
    torch.zeros_like(lead_times_collated) for _ in range(dist.get_world_size())
]
